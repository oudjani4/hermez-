...
async function applyOrder(order) {
  if (order.type === 'level') {
    const st = await mining.getState(order.uid);
    if (st.level >= order.target) { order.status = 'paid_unused'; return; }
    const r = await mining.setLevel(order.uid, order.target);
    if (!r.ok) { order.status = 'paid_apply_failed'; return; }
  } else {
